...
```mermaid
flowchart LR
    User -->|browser| React["React 18 SPA (Vite :5173)"]
    React -->|REST / JSON, dev-server proxy| Controller
    subgraph SpringBoot["Spring Boot (:8080)"]
        Controller["TodoController"] --> Service["TodoService"]
        Controller --> Handler["GlobalExceptionHandler"]
        Service --> Repository["TodoRepository (Spring Data JPA)"]
    end
    Repository --> H2[("H2 — file-based\n./data/tododb.mv.db")]
```
